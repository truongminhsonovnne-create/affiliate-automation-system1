 ...
 *
 * Shows all deals from a specific network (masoffer or accesstrade).
 * Sorted by hotness by default.
 */
...
function SourceDealsContent({ source }: { source: string }) {
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${config.accent} 0%, #fff 100%)`,
...
function SourceDealsPageWithParams() {
  ...
  return (
    <PublicLayout>
      <Suspense>
        <SourceDealsContent source={source} />
      </Suspense>
    </PublicLayout>
  );
}

export default function SourceDealsPage() {
  return (
    <Suspense fallback={
      <PublicLayout>
