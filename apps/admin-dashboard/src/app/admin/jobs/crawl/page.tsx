...
import { Globe, RefreshCw, Clock, Loader, CheckCircle, XCircle } from 'lucide-react';
...
import { useCrawlJobs, useDashboardOverview } from '@/lib/api/dashboardApi';
...
import { formatCrawlJobStatus, formatPlatform } from '@/lib/formatters/status';
import type { CrawlJobRecord } from '@/lib/types/api';
...
export default function CrawlJobsPage() {
  ...
  const filters = useFilterState({ status: '', platform: '', search: '' });
  const [searchInput, setSearchInput] = useState('');

  const { fetchCrawlJobs, fetchDashboardOverview } = useCrawlJobs();

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['crawl-jobs', pagination.page, pagination.pageSize, sort.sortKey, sort.sortDir, filters.filters],
    queryFn: () =>
      fetchCrawlJobs({
  ...
  const jobs = (data?.data?.data ?? []) as CrawlJobRecord[];
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: '160px',
      render: (item: CrawlJobRecord) => (
        <span className="text-sm text-gray-900 line-clamp-1">{item.keyword ?? '—'}</span>
      ),
    },
    {
      key: 'status',
      ...
      render: (item: CrawlJobRecord) => {
        const variant = item.status === 'completed' ? 'success'
          ...
          : 'neutral';
        return (
          <StatusBadge variant={variant} label={formatCrawlJobStatus(item.status)} size="sm" dot />
        );
      },
    },
    {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Jobs Crawl"
        description="Quản lý các job thu thập dữ liệu sản phẩm"
        icon={Globe}
        actions={
          <Button
            variant="secondary"
      ...
      {error ? (
        ...
        <DataTable
          ...
          loading={isLoading}
          ariaLabel="Danh sách job thu thập dữ liệu"
          pagination={
            meta
              ? {
                  page: pagination.page,
          ...
          stickyHeader
          emptyMessage="Không có job crawl nào"
        />
      )}
    </div>
  );
