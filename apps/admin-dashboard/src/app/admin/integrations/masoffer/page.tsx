/**
 * MasOffer Integration — Admin Dashboard Page
 *
 * Internal admin tool for monitoring and triggering MasOffer sync jobs.
...
export default function MasOfferIntegrationPage() {
  ...
  const fetchHealth = useCallback(async () => {
    ...
    setSyncError(null);
    try {
      const res = await fetch('/api/admin/masoffer/health', { credentials: 'include' });
      if (!res.ok) {
  ...
  const handleSync = async (type: 'offers' | 'campaigns') => {
    setSyncing(true);
    setSyncError(null);
    try {
      const res = await fetch('/api/admin/masoffer/sync', {
        method: 'POST',
        credentials: 'include',
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px' }}>
      <PageHeader
        title="MasOffer Integration"
        description="Monitor connection health, sync status, and trigger manual syncs for the MasOffer voucher pipeline."
        actions={
          <Button
            variant="secondary"
            size="sm"
        ...
        {loading && !health ? (
          ...
          <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-sm font-medium">API Credentials</div>
            ...
            {apiConnection.campaignCount !== undefined && (
              <div>
                ...
                <div className="text-xs" style={{ color: 'var(--gray-500)' }}>campaigns in MasOffer</div>
              </div>
            )}
